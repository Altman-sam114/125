...
截至当前工作树，唐宋迁移已推进到 v5.8q AppContainer 源头反馈中文化。当前主线是唐宋 v5.x / `main` 直推；v0.x 阿登内容只作为 legacy 架构地基和回归参考。
...
- 自动破城、完整外交纳土交割、完整漕运/粮队/仓储容量、完整胜利评分/统一结算、治理政策和完整发布级 UI 美术/截图验收仍未落地；v5.8q 仍是常见 AppContainer 反馈写入端中文化首轮，不是结构化 event payload、全项目写入端本地化、真 LLM 输出本地化、VoiceOver 实机验收或发布级 UI 验收。
...
下一轮优先继续 v5.8 / v5.9 发布候选收口：全局 accessibility 残留、截图/布局验收计划、玩家可见英文/raw id 残留扫尾和完整 artifact 复核。若回到功能开发，真实行动仍必须经 `Command` / `ZoneDirective -> WarCommandExecutor -> RuleEngine`，不得让 UI、事件或 Agent 直接改 `GameState`。
...
v5.8 已从 AI 面板默认主路径残留硬化、玩家态/开发态分层、外交面板读法硬化、战报日志读法硬化、MapEditor 默认路径硬化、文档产品定位收口、主游戏默认启动 fallback 硬化、唐宋将领注册表默认路径硬化、命令反馈/战报元数据硬化、检查面板 raw id / 目标状态硬化、AI 面板原始文本兜底、将领/州府面板 ASCII UI、兵力/粮道/地图数值标记硬化和 AppContainer 源头交互反馈中文化切入，但这不是完整发布候选审计：
...
- 全局 accessibility 文案、完整截图/VoiceOver/小屏布局验收仍未完成；结构化 event payload、真 LLM 输出本地化和全项目写入端本地化仍未完成，v5.8q 只覆盖常见 AppContainer 交互反馈。
