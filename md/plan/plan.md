...
下一轮可继续推进 v5.7 可玩闭环：目标州府地图高亮层、每回合战报摘要、势力选择/AI-only 口径、新局/重置包装或更精确的命令合法性提示；也可回到 v5.6 继续治理政策、完整胜利评分/统一结算、单国 tactical neutral 或归附后的控制权/部队处理方案。无论走哪条线，真实行动仍必须经 `Command` / `ZoneDirective -> WarCommandExecutor -> RuleEngine`，不得让 UI、事件或 Agent 直接改 `GameState`。
...
| v5.7 | 教程、剧本包装与可玩闭环 | 已开始：v5.7a 下一步提示、v5.7b 统一目标锚点、v5.7c 目标定位首轮 | 开局引导、势力选择、战报、新局/重置，让普通玩家能完成首发剧本 | `v5.7a_next_action_hint_record.md`、`v5.7b_objective_anchor_record.md`、`v5.7c_objective_focus_record.md`、`v5.7_playable_loop_record.md` |
...
v5.7 可玩闭环已从最小首屏提示切入。当前 v5.7a/v5.7b/v5.7c 已完成：
...
- v5.7c 是选区聚焦，不是自动镜头移动、路线指引或持续目标追踪系统。
- 未做截图、iPhone/iPad 横竖屏布局验收或 VoiceOver 验收，HUD 空间压力仍需后续 UI 验证。
- 尚未实现势力选择、每回合战报摘要、开局目标高亮、完整新局包装或持久化存档。

建议下一轮并发切片：

- UI owner：把目标锚点扩展为地图高亮层或镜头/缩放辅助，继续保持只读定位，不改规则。
