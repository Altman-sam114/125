...
截至当前工作树，唐宋迁移已推进到 v5.8p 兵力、粮道与数值标记 ASCII UI 硬化。当前主线是唐宋 v5.x / `main` 直推；v0.x 阿登内容只作为 legacy 架构地基和回归参考。
...
- 自动破城、完整外交纳土交割、完整漕运/粮队/仓储容量、完整胜利评分/统一结算、治理政策和完整发布级 UI 美术/截图验收仍未落地；v5.8p 仍是玩家可见显示硬化，不是结构化 event payload、完整写入端本地化、真 LLM 输出本地化、VoiceOver 实机验收或发布级 UI 验收。
...
v5.8 已从 AI 面板默认主路径残留硬化、玩家态/开发态分层、外交面板读法硬化、战报日志读法硬化、MapEditor 默认路径硬化、文档产品定位收口、主游戏默认启动 fallback 硬化、唐宋将领注册表默认路径硬化、命令反馈/战报元数据硬化、检查面板 raw id / 目标状态硬化、AI 面板原始文本兜底、将领/州府面板 ASCII UI 和兵力/粮道/地图数值标记硬化切入，但这不是完整发布候选审计：
