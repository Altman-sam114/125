...
- v5.3 已加入围城城防、修城、解围、地图围城 overlay 与 AI 围城指令首轮：`Command.besiege` 经 `RuleEngine` 登记 `SiegeState` 并损耗 `fortification`；`Command.repairFortification` 让守方军队在被围州府内消耗行动修城；`Command.relieveSiege` 让守方或友军削减围城 pressure，pressure 降到 0 时解除围城记录；`ZoneDirective.attack -> WarCommandExecutor` 可在目标敌控州府可围且无可攻击单位时生成底层 `Command.besiege`；Region 面板可读围城压力和城防，地图可从 `SiegeState` 只读绘制围城圈、压力和城防标签，城防归零后回合结算才把被围州府内守军压为 `lowSupply`。
...
下一轮优先继续 v5.3 小切片：粮道 UI / 漕运深化 / 围城后续破城或招降规则择一推进，仍必须走 `Command` / `ZoneDirective -> WarCommandExecutor -> RuleEngine`，不得让事件或 Agent 直接改 `GameState`。
...
| P1 | AI 围城指令 | 已完成首轮：`ZoneDirective.attack -> WarCommandExecutor` 在目标州府可围且无可攻击单位时生成底层 `Command.besiege` | `v5.3_rules_siege_grain_record.md`、后续 v5.4 记录 | 不让 AI 直接写 `GameState.siegeState` |
