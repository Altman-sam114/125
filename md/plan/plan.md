...
下一轮可继续推进 v5.6：设计治理政策、数据驱动胜利评价、单国 tactical neutral，或设计归附后的控制权/部队处理方案；仍必须走 `Command` / `ZoneDirective -> WarCommandExecutor -> RuleEngine`，不得让 UI、事件或 Agent 直接改 `GameState`。
...
v5.6 外交、归附、天命与治理不能只做 UI 或 Agent 文案。当前已完成 v5.6a 规则合同首轮、v5.6b 玩家入口/只读展示首轮、v5.6c AI 招抚辅助桥首轮、v5.6d 天命胜利评价桥首轮、v5.6e 关系投影同步首轮和 v5.6f 战术候选关系感知首轮：
...
- 唐宋 `VictoryRules` 读取关键州府 objective 控制方和 `MandateState`：宋统一胜利与割据生存都需要控制目标和天命阈值同时达标。
...
- `VictoryRules` 已有唐宋首轮胜利评价桥，但仍是硬编码关键州府与阈值；后续若要读取 JSON `victoryConditions`、评价等级或完整统一结算，还要继续设计。
