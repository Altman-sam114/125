...
- 自动破城、完整外交归附、AI 主动招降、完整漕运/粮队/仓储容量、唐宋专用胜利规则、天命/治理和发布级 UI 仍未落地。
- AI 默认人物和战术 raw case 仍有二战语义残留，需在 v5.4-v5.5 继续迁移。

下一轮优先继续 v5.3 小切片：漕运深化 / AI 主动招降 / 自动破城显示与胜利结算择一推进，仍必须走 `Command` / `ZoneDirective -> WarCommandExecutor -> RuleEngine`，不得让事件或 Agent 直接改 `GameState`。
