...
截至当前工作树，唐宋迁移已推进到 v5.8e MapEditor 默认路径读法硬化，v5.8f 正在做 README / plan / flow 文档产品定位收口。当前主线是唐宋 v5.x / `main` 直推；v0.x 阿登内容只作为 legacy 架构地基和回归参考。
...
- v5.8b 已完成 AI 面板玩家态/开发态分层首轮：`AgentPanelView` 在唐宋场景下默认显示军议摘要、命令执行状态和折叠的军议原文入口，diagnostics、错误原文和 raw JSON 改为折叠调试区；命令结果行改为“已执行 / 规则拒绝 / 映射失败”等摘要，找不到州府/防区名时显示“未命名州府 / 未命名方面”。该切片只改 UI 展示层，不改 AI 记录、raw JSON schema、compiler、executor、rules 或日志记录职责。
...
- 自动破城、完整外交纳土交割、完整漕运/粮队/仓储容量、完整胜利评分/统一结算、治理政策和完整发布级 UI 美术/截图验收仍未落地；v5.7i 只提供战报面板只读估算摘要。
...
下一轮优先继续 v5.8 / v5.9 发布候选收口：README/flow/update_log 最终发布口径、全局 accessibility 残留、截图/布局验收计划、主游戏 `DataLoader` fallback 策略和完整 artifact 复核。若回到功能开发，真实行动仍必须经 `Command` / `ZoneDirective -> WarCommandExecutor -> RuleEngine`，不得让 UI、事件或 Agent 直接改 `GameState`。
...
v5.8 已从 AI 面板默认主路径残留硬化、玩家态/开发态分层、外交面板读法硬化、战报日志读法硬化、MapEditor 默认路径硬化、文档产品定位收口、主游戏默认启动 fallback 硬化、唐宋将领注册表默认路径硬化和命令反馈/战报元数据硬化切入，但这不是完整发布候选审计：
...
- `AgentPanelView` 在唐宋场景下默认显示军议摘要、命令执行状态和折叠的军议原文入口；diagnostics、错误消息和 raw JSON 改为折叠调试区，不再默认铺开英文诊断、内部 id 或 schema key。
...
- 全局 accessibility 文案、完整截图/VoiceOver/小屏布局验收仍未完成；EventLog 的写入端结构化 payload 也仍未完成，本轮只做 UI / 工具 / 文档 / 默认启动路径硬化。

建议下一轮并发切片：

- UI owner：继续收口 Unit/Region inspector raw id、MapDisplayAdapter objectiveStatus、全局 accessibility、截图/布局验收和其他默认主路径残留。
