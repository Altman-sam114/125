...
截至当前工作树，唐宋迁移已推进到 v5.8an 外交面板复合行读屏硬化首轮。当前主线是唐宋 v5.x / `main` 直推；v0.x 阿登内容只作为 legacy 架构地基和回归参考。
...
- 自动破城、完整外交纳土交割、完整漕运/粮队/仓储容量、正式评分系统、治理政策和完整发布级 UI 美术/截图验收仍未落地；v5.8an 仍是外交面板 SwiftUI 可读语义硬化首轮，不是结构化 event payload、全项目写入端本地化、真 LLM 输出本地化、完整 VoiceOver 实机验收或发布级 UI 验收。
