...
下一轮可继续推进 v5.6：设计治理政策、完整胜利评分/统一结算、单国 tactical neutral，或设计归附后的控制权/部队处理方案；仍必须走 `Command` / `ZoneDirective -> WarCommandExecutor -> RuleEngine`，不得让 UI、事件或 Agent 直接改 `GameState`。
...
v5.6 外交、归附、天命与治理不能只做 UI 或 Agent 文案。当前已完成 v5.6a 规则合同首轮、v5.6b 玩家入口/只读展示首轮、v5.6c AI 招抚辅助桥首轮、v5.6d 天命胜利评价桥首轮、v5.6e 关系投影同步首轮、v5.6f 战术候选关系感知首轮、v5.6g 数据驱动胜利条件首轮和 v5.6h 胜负原因显示首轮：
...
- `VictoryRules` 已有唐宋 JSON `victoryConditions` 读取首轮，但只覆盖 objective 控制、回合门槛和天命阈值；评价等级、治理评分、单国胜负和完整统一结算仍需继续设计。
- v5.6h 只补胜负原因展示，不是完整胜利面板、结算页或每回合战报系统。
