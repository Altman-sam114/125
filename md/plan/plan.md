...
- v5.4 已开始 AI 军议显示桥首轮：`DirectiveType`、`CommandCategory`、`TacticName` 提供唐宋场景感知显示名；`AgentPanelView` 在唐宋场景下显示军议、方面军令、进军、骑军突进、合围、弓弩压制和死守城关等读法；底层 raw case、Codable schema 和 `ZoneDirective -> WarCommandExecutor -> RuleEngine` 权限边界不变。
...
- AI 默认人物、simulated rationale 和部分 raw JSON 仍有二战/英文语义残留，需在 v5.4-v5.5 继续迁移；本轮只完成面板显示桥。

下一轮优先继续 v5.4 或 v5.3 小切片：唐宋化 simulated marshal rationale / 漕运深化 / 围城结果显示与胜利结算择一推进，仍必须走 `Command` / `ZoneDirective -> WarCommandExecutor -> RuleEngine`，不得让事件或 Agent 直接改 `GameState`。
...
| P3 | 围城 AI 解释文案 | 已完成显示桥首轮：AI 面板在唐宋场景显示军议、方面军令和唐宋战术名；后续仍需让 simulated rationale 解释围城/修城/解围/招降原因 | `v5.3_rules_siege_grain_record.md`、`v5.4_agent_schema_record.md` | 不改变 AI 执行权限；解释文案不能替代规则结果 |
