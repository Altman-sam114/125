...
- UI 视觉目标是发布级唐宋历史策略质感：地图可读，州府、粮道、围城、接触带和军议可解释，不是调试面板；唐宋主线不得把古代战争直接套成现代连续战线。
- 作战模式长期方向是州府、关隘、行营、会战、袭扰、围城、粮道、仓储、漕运、士气、骑兵冲击和将领调度；可参考《十字军之王3》等中古战略游戏的集结、行军、围城、补给与 attrition 节奏，但落地必须继续经过 hex / region / command / rule 架构。
- iOS 主游戏、macOS 主游戏和 MapEditor 方向都保留；若某方向暂未完成，必须在阶段记录里写明。

## 3. 当前迁移进度快照

截至当前工作树，唐宋迁移已推进到 v5.8aq 唐宋整补粮道门槛首轮。当前主线是唐宋 v5.x / `main` 直推；v0.x 阿登内容只作为 legacy 架构地基和回归参考。
...
- v5.8am 已完成命令面板状态与反馈读屏硬化首轮：`CommandPanelView` 的军令状态、结束回合按钮状态和最近军令反馈补充读屏 label/value，减少状态与反馈被当成无上下文碎片朗读。
- v5.8an 已完成外交面板复合行读屏硬化首轮：`DiplomacyPanelView` 的诸国、关系和最近归附记录行补充合并读屏 label/value，减少国家副标题、关系状态和归附详情被拆散朗读。
- v5.8ao 已完成古代作战、粮道与接触带口径硬化首轮：`CombatRules` 提高唐宋骑军开阔地/道路进攻冲击加成，`CommandValidator` 阻止唐宋被围断粮军主动攻击或围城，`CommandValidationError.supplyBlocked` 与命令面板/战报显示桥补中文拒绝原因；地图图层、军队/州府检查面板和将领面板把唐宋玩家态“前线/部署”口径收束为接触带、敌我接触、接触州府、行营部署、行营辖区和固守城关。
- v5.8ap 已完成唐宋骑军追击与将领档案接触口径首轮：`CommandExecutor.resolveCombatResult` 在唐宋骑军使敌军自动退却时，若战场是平原，或攻守相邻地格存在道路的丘陵条件，追加基于本次兵力伤害的追击损耗；城池、关隘、山林和山地不触发追击。`GeneralProfileView` 的旧英文履历 fallback 改为敌我接触口径，不再默认说现代“战线形势”。
- v5.8aq 已完成唐宋整补粮道门槛首轮：`CommandValidator.validateRecoveryCommand` 在唐宋场景下要求整补军队处于 `supplied`，缺粮或被围军队返回 `CommandValidationError.supplyRecoveryBlocked`，显示为“粮道不通，不可整补”；legacy 阿登路径保持原有整补校验。
...
- 自动破城、完整外交纳土交割、完整漕运/粮队/仓储容量、正式评分系统、治理政策和完整发布级 UI 美术/截图验收仍未落地；v5.8aq 仍只是古代作战模型小切片，不是彻底移除内部 FrontLine / FrontZone schema，不是 CK3 式战争系统完成，也不是结构化 event payload、全项目写入端本地化、真 LLM 输出本地化、完整 VoiceOver 实机验收或发布级 UI 验收。
...
├── v5.8aj_event_log_victory_objective_accessibility_record.md # 已创建：战报胜利目标行读屏硬化
    ...
    ├── v5.8am_command_panel_status_feedback_accessibility_record.md # 已创建：命令面板状态与反馈读屏硬化
    ├── v5.8an_diplomacy_row_accessibility_record.md # 已创建：外交面板复合行读屏硬化
    ├── v5.8ao_ancient_combat_supply_contact_record.md # 已创建：古代作战、粮道与接触带口径硬化
    ├── v5.8ap_cavalry_pursuit_contact_record.md # 已创建：唐宋骑军追击与将领档案接触口径
    ├── v5.8aq_supply_recovery_gate_record.md # 已创建：唐宋整补粮道门槛
