...
截至当前工作树，唐宋迁移已推进到 v5.8j 检查面板 raw id 与目标状态硬化。当前主线是唐宋 v5.x / `main` 直推；v0.x 阿登内容只作为 legacy 架构地基和回归参考。
...
- 自动破城、完整外交纳土交割、完整漕运/粮队/仓储容量、完整胜利评分/统一结算、治理政策和完整发布级 UI 美术/截图验收仍未落地；v5.7i 只提供战报面板只读估算摘要，v5.8j 只处理检查面板显示名和目标状态。
...
| v5.8 | 发布候选硬化 | 已完成 v5.8a-v5.8j 首轮：AI 面板、玩家态/开发态分层、外交面板、战报日志、MapEditor 默认路径、文档定位、主游戏默认启动 fallback、唐宋将领注册表默认路径、命令反馈/战报元数据和检查面板 raw id / 目标状态硬化；完整 RC 未完成 | 玩家可见残留扫描、资源授权、性能、文档口径和云端 artifact 验收收口 | `v5.8a_ai_panel_default_path_hardening_record.md`、`v5.8b_ai_panel_debug_player_dev_split_record.md`、`v5.8c_diplomacy_panel_default_path_hardening_record.md`、`v5.8d_event_log_default_path_hardening_record.md`、`v5.8e_mapeditor_default_path_hardening_record.md`、`v5.8f_docs_product_positioning_record.md`、`v5.8g_main_game_default_loader_hardening_record.md`、`v5.8h_tangsong_general_registry_hardening_record.md`、`v5.8i_command_feedback_event_metadata_hardening_record.md`、`v5.8j_inspector_raw_id_hardening_record.md`、`v5.8_release_candidate_audit.md` |
...
v5.8 已从 AI 面板默认主路径残留硬化、玩家态/开发态分层、外交面板读法硬化、战报日志读法硬化、MapEditor 默认路径硬化、文档产品定位收口、主游戏默认启动 fallback 硬化、唐宋将领注册表默认路径硬化、命令反馈/战报元数据硬化和检查面板 raw id / 目标状态硬化切入，但这不是完整发布候选审计：
