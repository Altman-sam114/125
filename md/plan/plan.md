...
下一轮优先继续 v5.3 小切片：城防耐久 / 修城 / 围城解除 / AI 围城指令 / 粮道 UI / 漕运深化择一推进，仍必须走 `Command` / `ZoneDirective -> WarCommandExecutor -> RuleEngine`，不得让事件或 Agent 直接改 `GameState`。
...
|---|---|---|---|---|
| P0 | 城防耐久 | 在 `SiegeState` / `SiegeRecord` 上补城防或墙体完整度，让围城压力有可见积累目标 | `v5.3_rules_siege_grain_record.md`、`md/flow/flow.md`、`md/flow/flowchart.md`、本文、`update_log.md` | 不自动改 hex/region 控制权，不绕过移动占领 |
| P0 | 修城命令 | 通过 `Command.repairFortification` 或等价命令让守方消耗行动/资源恢复城防 | 同上 | 不把修城写成 UI 直接改状态 |
| P1 | 解围 / 驰援 | 增加合法解围或围城维持规则，明确何时移除 siege record | 同上 | 不用事件直接删除敌军或改变归属 |
...
- 围城至少具备登记、压力、城防/断粮效果、解围/失效、面板读法。
