...
v5.8 已从 AI 面板默认主路径残留硬化、玩家态/开发态分层、外交面板读法硬化、战报日志读法硬化、MapEditor 默认路径硬化和文档产品定位收口切入，但这不是完整发布候选审计：
...
- 主游戏 `DataLoader` legacy fallback、全局 accessibility 文案、完整截图/VoiceOver/小屏布局验收仍未完成；EventLog 的写入端结构化 payload 也仍未完成，本轮只做 UI / 工具 / 文档默认路径硬化。

建议下一轮并发切片：

- UI owner：继续收口全局 accessibility、截图/布局验收和其他默认主路径残留。
