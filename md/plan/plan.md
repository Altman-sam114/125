...
后续 Agent 接手唐宋迁移时，先用本文判断“当前做到哪一层、下一步该改哪些文档”，再回到 `md/flow/flow.md`、`md/flow/flowchart.md` 和源码确认真实实现。本文不是功能验收报告，也不能替代 GitHub Actions artifact；它只作为 `md/` 目录的大纲、阶段索引和交接清单。
...
- v5.3 已加入围城城防与修城首轮：`Command.besiege` 经 `RuleEngine` 登记 `SiegeState` 并损耗 `fortification`；`Command.repairFortification` 让守方军队在被围州府内消耗行动修城；Region 面板可读围城压力和城防，城防归零后回合结算才把被围州府内守军压为 `lowSupply`。
...
- 完整围城链、解围、自动破城、招降、完整漕运/粮队/仓储容量、唐宋专用胜利规则、天命/归附/治理和发布级 UI 仍未落地。
- AI 默认人物和战术 raw case 仍有二战语义残留，需在 v5.4-v5.5 继续迁移。

下一轮优先继续 v5.3 小切片：解围 / 围城 overlay / AI 围城指令 / 粮道 UI / 漕运深化择一推进，仍必须走 `Command` / `ZoneDirective -> WarCommandExecutor -> RuleEngine`，不得让事件或 Agent 直接改 `GameState`。
...
| P1 | 解围 / 驰援 | 增加合法解围或围城维持规则，明确何时主动移除或削弱 siege record | 同上 | 不用事件直接删除敌军或改变归属 |
| P1 | 围城 UI/overlay | 地图或 Region 面板显示围城圈、压力、城防和围困军队摘要 | `README.md`、`md/flow/*`、`v5.3_rules_siege_grain_record.md` | 不把视觉层写成规则层 |
| P1 | AI 围城指令 | 让 `WarCommandExecutor` / ZoneDirective 能选择围城目标，仍产出底层 `Command.besiege` | `v5.3_rules_siege_grain_record.md`、后续 v5.4 记录 | 不让 AI 直接写 `GameState.siegeState` |
| P2 | 漕运 / 粮道读法 | 让补给路径、粮仓源和断粮风险在 UI 或日志里可解释 | `md/flow/*`、`README.md`、`v5.3_rules_siege_grain_record.md` | 不新增复杂仓储系统，除非另开切片 |

v5.3 收口标准：

- 默认唐宋生产、资源、战斗、补给和围城链路没有主要二战显示词。
- 围城至少具备登记、压力、城防/断粮效果、修城、解围/失效、面板读法。
...
├── v5.3_rules_siege_grain_record.md        # 已创建：生产/府库、兵种战斗、粮道供给、围城城防和修城首轮
