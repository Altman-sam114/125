...
v5.8 已从 AI 面板默认主路径残留硬化、玩家态/开发态分层、外交面板读法硬化、战报日志读法硬化、MapEditor 默认路径硬化、文档产品定位收口和主游戏默认启动 fallback 硬化切入，但这不是完整发布候选审计：
...
- UI owner：继续收口 legacy `generals.json` 在唐宋将领面板中的显示、命令面板 raw validation、inspector raw id、全局 accessibility、截图/布局验收和其他默认主路径残留。
