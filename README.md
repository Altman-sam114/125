...
> **v5.6g 最新小切片：** v5.6d 的唐宋胜利评价桥已从硬编码中文州府名推进为数据驱动读取当前唐宋 JSON 的 `victoryConditions`。该切片不新增治理政策、不实现归附纳土、不转换部队、不新增完整评分/胜利面板，也不改变最终 `VictoryRules` 权威。
