...
> **v5.8c 最新小切片：** 唐宋外交面板默认主路径读法硬化首轮，减少外交状态、集团、君主、方面、州府 fallback 中的英文和 raw id 残留；这只是只读显示桥，不改变外交、天命、关系投影或规则。
...
└── prompt/
    ...
    ├── v5.0-唐宋迁移/
    │   唐宋 v5.0-v5.9 总提示词、v5.0 审计合同、v5.7a 下一步提示、v5.7b 目标锚点、v5.7c 目标定位、v5.7d 地图目标高亮、v5.7e 战报摘要、v5.7f 新局身份包装、v5.7g 高亮数量提示、v5.7h 亲征/观战入口、v5.7i 结算预览/评分估算、v5.7j 合法性提示、v5.7k 检查面板读法、v5.7l 将领面板读法、v5.7m 常驻 tooltip 读法、v5.8a AI 面板默认主路径硬化、v5.8b AI 面板玩家态/开发态分层、v5.8c 外交面板读法硬化等阶段记录
    ├── v2.0-三国迁移/
    ├── v3.0-拿战迁移/
    ├── v3.0-隋唐迁移/
    ├── v4.0-明末迁移/
