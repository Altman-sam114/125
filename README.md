...
> **v5.8g 最新小切片：** 主游戏 `DataLoader.loadInitialGameState()` 默认启动不再静默回退 legacy 阿登资源；唐宋默认 JSON 缺失或校验失败时返回唐宋错误态并写入中文日志，显式 legacy 阿登入口仍保留给历史回归参考。该切片只改默认启动容错，不改变 JSON/Codable schema、规则系统、MapEditor 读写和显式 `loadGameState(ardennes...)` 路径。
...
| v5.8a-v5.8g | 进行中 | AI 面板、外交面板、战报日志、MapEditor 默认路径、README/plan/flow 文档定位和主游戏 DataLoader 默认启动 fallback 已做默认主路径硬化首轮。完整 RC 审计仍未完成。 |
