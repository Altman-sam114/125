...
> **v5.8af 最新小切片：** 将领军令面板指标与列表读屏继续硬化：`GeneralCommandPanelView` 唐宋路径下的忠诚/军心指标补“满百”读屏值，亲征干预显示“N 次”，所属军队行补兵力读屏值，已拟军令行补“已拟军令：进攻/固守”和目标值。该切片只改将领军令面板显示和 SwiftUI 可读语义，不改变 `GeneralAssignment`、`Division`、`PlayerPlannedOperation`、命令按钮、AI 决策、规则或 Codable schema；完整 VoiceOver 实机、截图布局和发布级 UI 验收仍未完成。
...
**你接手时的代码库状态：**
- 当前主线是唐宋 v5.x 迁移，默认剧本为 `jianlong_960_unification`；v5.8af 已继续收口将领军令面板忠诚/军心、亲征干预、所属军队和已拟军令读屏语义。最新推送按 GitHub Actions 云端验证与 artifact 核对为准。
