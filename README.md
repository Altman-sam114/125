...
> **v5.8ai 最新小切片：** 战报胜负与评分卡片读屏继续硬化：`EventLogView` 唐宋路径下的“胜负”和“评分估算”卡片合并标题、胜负原因、分数、档位和说明读屏语义。该切片只改战报顶部胜负/评分卡片 SwiftUI 可读语义，不改变 `VictoryState`、`VictoryReason`、`VictoryRules.objectiveProgress(in:)`、评分估算算法、事件写入、命令、AI 决策、规则或 Codable schema；胜利目标行、完整 VoiceOver 实机、截图布局和发布级 UI 验收仍未完成。
...
**你接手时的代码库状态：**
- 当前主线是唐宋 v5.x 迁移，默认剧本为 `jianlong_960_unification`；v5.8ai 已继续收口战报胜负和评分估算卡片的胜负原因、分数、档位和说明读屏语义。最新推送按 GitHub Actions 云端验证与 artifact 核对为准。
