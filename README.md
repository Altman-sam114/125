...
> **v5.8ah 最新小切片：** 战报摘要卡片读屏继续硬化：`EventLogView` 唐宋路径下的“本回合战报 / 最近战报”摘要卡片合并标题、回合、汇总文字和重点条目读屏语义，减少摘要卡片被拆成多个碎片焦点。该切片只改战报摘要卡片 SwiftUI 可读语义，不改变 `GameLogEntry`、`TurnReportSummary` 派生逻辑、`TangSongEventLogMessage` 显示桥、事件写入、命令、AI 决策、规则或 Codable schema；完整 VoiceOver 实机、截图布局和发布级 UI 验收仍未完成。
...
**你接手时的代码库状态：**
- 当前主线是唐宋 v5.x 迁移，默认剧本为 `jianlong_960_unification`；v5.8ah 已继续收口本回合/最近战报摘要卡片的标题、回合、汇总和重点条目读屏语义。最新推送按 GitHub Actions 云端验证与 artifact 核对为准。
