...
> **v5.8ak 最新小切片：** HUD 目标与下一步提示读屏继续硬化：`HUDView` 唐宋路径下的“目标”摘要文本合并为“目标提示”读屏 value，同时保留每个关键州府按钮的独立焦点；“下一步”提示卡片合并为“下一步提示”读屏 value。该切片只改 HUD SwiftUI 可读语义，不改变 `VictoryRules.objectiveProgress(in:)`、目标锚点生成、地图聚焦、下一步提示计算、命令、AI 决策、规则或 Codable schema；完整 VoiceOver 实机、截图布局和发布级 UI 验收仍未完成。
...
**你接手时的代码库状态：**
- 当前主线是唐宋 v5.x 迁移，默认剧本为 `jianlong_960_unification`；v5.8ak 已继续收口 HUD 目标提示摘要和下一步提示卡片的读屏语义。最新推送按 GitHub Actions 云端验证与 artifact 核对为准。
