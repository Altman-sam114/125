...
> **v5.8am 最新小切片：** 命令面板状态与反馈读屏继续硬化：`CommandPanelView` 唐宋路径下的军令状态、结束回合按钮状态和最近军令反馈补充 accessibility label/value，让读屏按“军令状态 / 军令反馈”理解状态与反馈。该切片只改命令面板 SwiftUI 可读语义，不改变 `Command`、`CommandValidator`、`RuleEngine`、按钮启停、命令反馈写入或 Codable schema；完整 VoiceOver 实机、截图布局和发布级 UI 验收仍未完成。
...
**你接手时的代码库状态：**
- 当前主线是唐宋 v5.x 迁移，默认剧本为 `jianlong_960_unification`；v5.8am 已继续收口命令面板状态与反馈的读屏语义。最新推送按 GitHub Actions 云端验证与 artifact 核对为准。
