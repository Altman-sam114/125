...
> **v5.8ap 最新小切片：** 唐宋骑军追击与将领档案接触口径首轮：`CommandExecutor.resolveCombatResult` 在唐宋骑军使敌军自动退却时，若退却发生在平原，或攻守相邻地格存在道路的丘陵环境，追加基于本次兵力伤害的追击损耗；城池、关隘、山林和山地不触发该追击。将领档案旧英文履历 fallback 同步改为“敌我接触形势”，不再说现代“战线形势”。该切片不新增骑军 AI、粮队、会战系统或 CK3 式战争系统，只是古代作战模型的下一处规则小步。
...
**你接手时的代码库状态：**
- 当前主线是唐宋 v5.x 迁移，默认剧本为 `jianlong_960_unification`；v5.8ao 已把古代作战模型方向写入 README / AGENTS，并完成骑军冲击、断粮出击限制和接触带口径首轮；v5.8ap 继续加入唐宋骑军自动退却追击和将领档案接触口径首轮。最新推送按 GitHub Actions 云端验证与 artifact 核对为准。
