...
统治者 agent 不在 v0.5 当前主链路中。v5.6a 已先建立 `MandateState`、`PacificationRecord` 和 `Command.proposeSubmission` 规则合同，但国家级外交关系仍是 `DiplomacyState` 投影，战术敌我仍由 legacy `Faction.germany` / `Faction.allies` 与 `TurnOrderState.relations` 决定。后续如要加入统治者 agent、单国 tactical neutral、完整纳土或治理政策，仍必须保持底层战争规则收口到 `Command` / `ZoneDirective`、`WarCommandExecutor` 和 `RuleEngine`。
