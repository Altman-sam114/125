...
> **v5.8aj 最新小切片：** 战报胜利目标行读屏继续硬化：`EventLogView` 唐宋路径下的“胜利目标”进度行合并目标标题、达成/推进中状态、进度摘要和详情读屏语义。该切片只改战报胜利目标行 SwiftUI 可读语义，不改变 `VictoryState`、`VictoryRules.objectiveProgress(in:)`、`VictoryObjectiveProgress` 计算逻辑、胜利目标排序、可见文案、事件写入、命令、AI 决策、规则或 Codable schema；完整 VoiceOver 实机、截图布局和发布级 UI 验收仍未完成。
...
**你接手时的代码库状态：**
- 当前主线是唐宋 v5.x 迁移，默认剧本为 `jianlong_960_unification`；v5.8aj 已继续收口战报胜利目标行的标题、达成状态、进度摘要和详情读屏语义。最新推送按 GitHub Actions 云端验证与 artifact 核对为准。
