...
> **v5.7j 最新小切片：** 唐宋 HUD 下一步提示会对当前 UI 候选做有限规则预校验，优先提示“规则确认可执行”的地图或军令入口；这不是通用规则模拟器，也不替代真正提交后的 `RuleEngine` 执行。
...
└── prompt/
    ...
    ├── v5.0-唐宋迁移/
    │   唐宋 v5.0-v5.9 总提示词、v5.0 审计合同、v5.7a 下一步提示、v5.7b 目标锚点、v5.7c 目标定位、v5.7d 地图目标高亮、v5.7e 战报摘要、v5.7f 新局身份包装、v5.7g 高亮数量提示、v5.7h 亲征/观战入口、v5.7i 结算预览/评分估算、v5.7j 合法性提示等阶段记录
    ├── v2.0-三国迁移/
    ├── v3.0-拿战迁移/
    ├── v3.0-隋唐迁移/
    ├── v4.0-明末迁移/
