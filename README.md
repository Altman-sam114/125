...
> **v5.6h 最新小切片：** v5.6g 的数据驱动胜负结果现在有玩家可读的原因显示：例如宋胜利会显示“关键州府与天命达标”。该切片不新增胜利条件、不写权威战报事件、不改变 `VictoryRules`、不做完整胜利面板。
