...
> **v5.8ab 小切片：** MapEditor 画布符号继续硬化：画布粮源标记从“补”改为“粮”，军队模板标记从单字 `禁/骑/弩/械/守/州/军` 改为“禁军、骑军、弩兵、器械、守军、州军、军队”，底图 SpriteKit 节点不再写可见/调试名称。该切片只改 MapEditor SpriteKit 画布显示标签，不改变 `MapEditorDocument`、单位模板 id、导出 JSON schema、资源桥路径、画布交互、快捷键处理、主游戏规则或 Codable raw 值；完整截图、布局和符号系统验收仍未完成。

> **v5.8ac 小切片：** 军议与方面军令反馈继续硬化：`AppContainer` 唐宋路径的方面军令部分执行摘要从 `N/M` 改为“已执行 N 道，未执行 M 道”，`CommandPanelView` 对旧英文 `General order executed...` 摘要做完整中文兜底，`AgentPanelView` 玩家态把“军议原文”改为“军议详文”。该切片只改玩家可见反馈和显示桥，不改变 `Command`、`ZoneDirective`、`WarDirectiveRecord`、`AgentDecisionRecord.rawJSON`、命令执行、AI 决策、规则或 Codable schema；完整全项目写入端本地化和发布级 UI 验收仍未完成。

> **v5.8ad 小切片：** 府库军备队列可读性继续硬化：`EconomyPanelView` 唐宋路径的军备队列未就绪项显示“尚需 N 回合”，府库收入/耗粮指标改为“本回合丁口 / 本回合钱帛 / 本回合耗粮”，军备按钮补“下达军备令”读屏动作名，队列行补完整 accessibility label/value。该切片只改府库面板显示和 SwiftUI 可读语义，不改变 `ProductionOrder`、`ProductionKind`、经济规则、队列执行、资源扣除、命令或 Codable schema；完整 VoiceOver 实机、截图布局和发布级 UI 验收仍未完成。

> **v5.8ae 小切片：** HUD 指标与队列读法继续硬化：`HUDView` 唐宋路径下把丁口、钱帛、粮草库存从裸数字改为“丁口 N / 钱帛 N / 粮草 N”，军备队列显示为“暂无军备”或“军备队列 N 项”，并让 HUD 指标行合并 accessibility label/value。该切片只改 HUD 显示和 SwiftUI 可读语义，不改变 `EconomyState`、生产队列、资源规则、命令、胜利规则或 Codable schema；完整 VoiceOver 实机、截图布局和发布级 UI 验收仍未完成。

> **v5.8ah 小切片：** 战报摘要卡片读屏继续硬化：`EventLogView` 唐宋路径下的“本回合战报 / 最近战报”摘要卡片合并标题、回合、汇总文字和重点条目读屏语义，减少摘要卡片被拆成多个碎片焦点。该切片只改战报摘要卡片 SwiftUI 可读语义，不改变 `GameLogEntry`、`TurnReportSummary` 派生逻辑、`TangSongEventLogMessage` 显示桥、事件写入、命令、AI 决策、规则或 Codable schema；完整 VoiceOver 实机、截图布局和发布级 UI 验收仍未完成。

> **v5.8ai 小切片：** 战报胜负与评分卡片读屏继续硬化：`EventLogView` 唐宋路径下的“胜负”和“评分估算”卡片合并标题、胜负原因、分数、档位和说明读屏语义。该切片只改战报顶部胜负/评分卡片 SwiftUI 可读语义，不改变 `VictoryState`、`VictoryReason`、`VictoryRules.objectiveProgress(in:)`、评分估算算法、事件写入、命令、AI 决策、规则或 Codable schema；完整 VoiceOver 实机、截图布局和发布级 UI 验收仍未完成。

> **v5.8aj 小切片：** 战报胜利目标行读屏继续硬化：`EventLogView` 唐宋路径下的“胜利目标”进度行合并目标标题、达成/推进中状态、进度摘要和详情读屏语义。该切片只改战报胜利目标行 SwiftUI 可读语义，不改变 `VictoryState`、`VictoryRules.objectiveProgress(in:)`、`VictoryObjectiveProgress` 计算逻辑、胜利目标排序、可见文案、事件写入、命令、AI 决策、规则或 Codable schema；完整 VoiceOver 实机、截图布局和发布级 UI 验收仍未完成。

> **v5.8ak 小切片：** HUD 目标与下一步提示读屏继续硬化：`HUDView` 唐宋路径下的“目标”摘要文本合并为“目标提示”读屏 value，同时保留每个关键州府按钮的独立焦点；“下一步”提示卡片合并为“下一步提示”读屏 value。该切片只改 HUD SwiftUI 可读语义，不改变 `VictoryRules.objectiveProgress(in:)`、目标锚点生成、地图聚焦、下一步提示计算、命令、AI 决策、规则或 Codable schema；完整 VoiceOver 实机、截图布局和发布级 UI 验收仍未完成。

> **v5.8al 小切片：** AI 面板军令结果行读屏继续硬化：`AgentPanelView` 唐宋路径下的“军令结果”列表行合并命令标题和执行状态读屏语义，读作“军令结果：行军 / 已执行”等完整状态。该切片只改 AI 面板 SwiftUI 可读语义，不改变 `AgentDecisionRecord`、`CommandResultSummary`、`WarDirectiveRecord`、`ZoneDirective`、命令执行、AI 决策、规则或 Codable schema；完整 VoiceOver 实机、截图布局和发布级 UI 验收仍未完成。

> **v5.8am 小切片：** 命令面板状态与反馈读屏继续硬化：`CommandPanelView` 唐宋路径下的军令状态、结束回合按钮状态和最近军令反馈补充 accessibility label/value，让读屏按“军令状态 / 军令反馈”理解状态与反馈。该切片只改命令面板 SwiftUI 可读语义，不改变 `Command`、`CommandValidator`、`RuleEngine`、按钮启停、命令反馈写入或 Codable schema；完整 VoiceOver 实机、截图布局和发布级 UI 验收仍未完成。

> **v5.8an 最新小切片：** 外交面板复合行读屏继续硬化：`DiplomacyPanelView` 唐宋路径下的诸国、关系和最近归附记录行补充 accessibility label/value，让读屏按国家条目、关系状态和归附结果理解复合行。该切片只改外交面板 SwiftUI 可读语义，不改变 `DiplomacyState`、`MandateState`、`PacificationRecord`、`Command.proposeSubmission`、外交关系投影、命令、规则、AI 决策、控制权交割或 Codable schema；完整 VoiceOver 实机、截图布局和发布级 UI 验收仍未完成。

> **v5.8ao 最新小切片：** 唐宋作战模型方向首轮收口：`CombatRules` 把唐宋骑军在开阔地/道路进攻的冲击加成提高，`CommandValidator` 阻止唐宋被围断粮军主动攻击或围城并显示“粮道断绝，不可主动出击”，地图图层、军队/州府检查面板和将领面板把唐宋玩家态“前线/部署”口径收束为接触带、敌我接触、接触州府、行营部署、行营辖区和固守城关。该切片不删除内部 legacy `FrontLine` / `FrontZone` schema，也不等于完成 CK3 式作战模型；后续仍需继续研究州府、关隘、行营、粮草、骑军迂回、会战与围城节奏。

> **v5.8ap 小切片：** 唐宋骑军追击与将领档案接触口径首轮：`CommandExecutor.resolveCombatResult` 在唐宋骑军使敌军自动退却时，若退却发生在平原，或攻守相邻地格存在道路的丘陵环境，追加基于本次兵力伤害的追击损耗；城池、关隘、山林和山地不触发该追击。将领档案旧英文履历 fallback 同步改为“敌我接触形势”，不再说现代“战线形势”。该切片不新增骑军 AI、粮队、会战系统或 CK3 式战争系统，只是古代作战模型的下一处规则小步。

> **v5.8aq 最新小切片：** 唐宋整补粮道门槛首轮：`CommandValidator.validateRecoveryCommand` 在唐宋场景下要求整补军队处于 `supplied`，缺粮或被围军队会返回 `CommandValidationError.supplyRecoveryBlocked`，显示为“粮道不通，不可整补”。该切片让粮道从显示和攻势限制继续推进为整补前置条件；legacy 阿登路径保持原有整补校验，不新增粮队、仓储容量、低补给 attrition 或围城破城自动 encircled。
...
- 《钢铁雄心4》：大战略、省份占领、补给、生产、国家管理（只作战略层参照，不照搬现代连续前线）
- EasyTech《钢铁命令》：战役推进、将领、战术操作
- 《世界征服者4》：移动端轻量化策略体验
- 《十字军之王3》等中古战略游戏：集结、行军、围城、骑兵、封建军队调度、补给与 attrition 的节奏参考

**唐宋作战模型方向：**

- 古代/中古战争不应直接套成现代连续战线。底层 legacy `FrontLine` 可继续作为“敌我动态接触”和部署派生数据，但唐宋玩家态优先显示为接触带、敌我接触、接触州府、行营部署和固守城关。
- 骑兵是唐宋野战、追击、突袭和道路机动作战的关键力量；开阔地与道路应显著放大骑军冲击，城池、关隘、山林和围城环境应压制骑兵优势。
- 粮草、粮源、仓城、道路、跨河、山林、围城和被围状态是作战核心变量。断粮军队不应和有粮军队一样主动攻城、出击或持久作战。
- 后续作战模式应继续研究州府控制、行营调度、关隘固守、围城逼降、粮道袭扰、骑军迂回、会战窗口和将领命令，而不是只做二战术语替换。
...
Dynamic Theater State（运行时动态战区 / 随 hex 推进变化）
  ↓ 动态 hex 邻接
FrontLine / FrontSegment（接触带派生层，legacy 命名；按动态战区接触生成）
  ↓
...
- **v0.358 之后的关键语义**：
  ...
  - 唐宋玩家态不按现代连续战线理解；legacy `FrontLine` = 我方动态战区与敌方动态战区的 hex 邻接接触，按 region 形成接触带 `FrontSegment`。
...
| v5.8a-v5.8aq | 进行中 | AI 面板、外交面板、战报日志、MapEditor 默认路径、README/plan/flow 文档定位、主游戏 DataLoader 默认启动 fallback、唐宋将领注册表默认路径、命令反馈/战报元数据、检查面板 raw id / 目标状态、命令/战报 raw 英文兜底、将领计划摘要、固定英文 UI、外交 Latin/ASCII 显示、AI 面板原始文本兜底、将领/州府面板 ASCII 显示、兵力/粮道/地图数值标记、AppContainer 源头交互反馈、胜利/粮道/军议摘要、MapEditor raw UI、accessibility / VoiceOver 可读文案、控件状态提示、MapEditor 错误/输入可访问性、主棋盘 VoiceOver 自定义动作、面板控件 accessibility/fallback、tooltip 与检查面板读法、将领档案 accessibility/fallback、MapEditor 画布 value/底图控件、画布符号、军议/方面军令反馈、府库军备队列可读性、HUD 指标/队列读法、将领军令面板读屏、战报行整行读屏、战报摘要卡片读屏、战报胜负/评分卡片读屏、战报胜利目标行读屏、HUD 目标/下一步提示读屏、AI 面板军令结果行读屏、命令面板状态/反馈读屏、外交面板复合行读屏、唐宋骑军冲击/断粮出击限制、接触带口径、骑军自动退却追击、将领档案接触口径和整补粮道门槛硬化已做默认主路径首轮。完整 RC 审计仍未完成。 |
...
**你接手时的代码库状态：**
- 当前主线是唐宋 v5.x 迁移，默认剧本为 `jianlong_960_unification`；v5.8ao 已把古代作战模型方向写入 README / AGENTS，并完成骑军冲击、断粮出击限制和接触带口径首轮；v5.8ap 继续加入唐宋骑军自动退却追击和将领档案接触口径首轮；v5.8aq 继续加入唐宋整补粮道门槛。最新推送按 GitHub Actions 云端验证与 artifact 核对为准。
...
- 接触带 UI 和 AI target 选择必须基于动态 hex 邻接；历史测试 fixture / 语义文档也必须构造真实相邻 hex，不能只声明 region 邻接。
