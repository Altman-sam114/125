...
> **v5.8ag 最新小切片：** 战报行整行读屏继续硬化：`EventLogView` 唐宋路径下的最近战报列表行合并分类、回合/势力/阶段元数据和正文读屏语义，读作“战报：战斗/粮道/州府”等分类并把正文作为 value。该切片只改战报列表行 SwiftUI 可读语义，不改变 `GameLogEntry`、`TangSongEventLogMessage` 显示桥、事件写入、命令、AI 决策、规则或 Codable schema；完整 VoiceOver 实机、截图布局和发布级 UI 验收仍未完成。
...
**你接手时的代码库状态：**
- 当前主线是唐宋 v5.x 迁移，默认剧本为 `jianlong_960_unification`；v5.8ag 已继续收口战报列表行的分类、元数据和正文整行读屏语义。最新推送按 GitHub Actions 云端验证与 artifact 核对为准。
